...
class TestTools(unittest.TestCase):
    ...
    def test_list_directory_not_found(self):
        ...
        self.assertEqual(result, f"Error: El directorio '/tmp/nonexistent_dir' no existe.")
    ...
    def test_update_long_term_memory_success(self):
        ...
        self.assertEqual(
            result,
            f"Memoria a largo plazo actualizada exitosamente en {self.test_memory_file}.",
        )
...
class TestAgentServer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._agent_memory_file_patcher_server = patch.object(
            agent_server, "AGENT_MEMORY_FILE", self.test_memory_file
        )
    ...
    def test_load_long_term_memory_success(self, mock_open_func):
        ...
        mock_open_func.assert_called_once_with(
            self.test_memory_file, "r", encoding="utf-8"
        )
    ...
    def test_load_long_term_memory_not_found(self, mock_open_func):
        content = load_long_term_memory()
        self.assertIn(
            "Advertencia: No se encontró el archivo de memoria del agente.", content
        )
    ...
    def test_build_system_prompt(self):
        ...
        prompt = build_system_prompt(
            long_term_memory, conversation_history, user_request
        )
    ...
    @patch(
        "agent_server.AVAILABLE_TOOLS", new_callable=dict
    )  # Patch with a real dictionary
    def test_execute_tool_success(self, mock_available_tools):
        # Create individual mocks for each tool
        mock_run_shell = MagicMock(
            return_value={"stdout": "ls output", "stderr": "", "exit_code": 0}
        )
        mock_read_file = MagicMock(return_value="file content")
        mock_write_file = MagicMock(
            return_value="Archivo /tmp/test.txt escrito exitosamente."
        )
        mock_list_dir = MagicMock(return_value=json.dumps(["file1", "file2"]))
        mock_update_memory = MagicMock(
            return_value="Memoria a largo plazo actualizada exitosamente en /tmp/test_agent_cli_dir_server/agent_memory.md."
        )
        ...
        self.assertEqual(
            json.loads(result), {"stdout": "ls output", "stderr": "", "exit_code": 0}
        )
        ...
        # Test write_file
        result = execute_tool(
            "write_file", {"path": "/tmp/test.txt", "content": "new content"}
        )
        ...
        self.assertEqual(
            result,
            "Memoria a largo plazo actualizada exitosamente en /tmp/test_agent_cli_dir_server/agent_memory.md.",
        )
